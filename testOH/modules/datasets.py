...
# --------------------------------------------------------------------------------
# 여기 이따가 확인해봐야함 label_num으로 train test split중
def get_harbor_idx(root, train=True, is_label=True , split_rate=0.8):
    if train:
        if is_label:
            ...
            for c in classes:
                ...
                split_idx = int(len(matched_idx) * split_rate)
                train_idx.extend(matched_idx[:split_idx])
                valid_idx.extend(matched_idx[split_idx:])
...
class BuildDataLoader:
    def __init__(self, num_labels, dataset_path, batch_size, split_rate):
        ...
        self.train_l_idx, self.valid_l_idx = get_harbor_idx(self.data_path, train=True, is_label=True, split_rate=self.split_rate)
